...
users = [
  {
    username: "evzonas",
    password: "$2b$10$zqswotaqEpOctOSL1Ox3tOuOkRhQXLlhLlwmVIx/sH7NnQcZ8IMs.",
    email: "evzonas@evzonas.net",
    id: "1234",
  },
];
...
initializePassport(
  passport,
  (email) => {
    userObject = users.find((user) => user.email === email);
  ...
  (id) => {
    userObject = users.find((user) => user.id === id);
    return userObject;
    // const response = axios.get(`http://localhost:3001/users`);
    // return response.data[0];
  }
...
app.get("/", (req, res) => {
  ...
  res.render("register.ejs", { email: req.user.email });
});
app.post(
  "/loginUser/",
...
app.post("/users/", async (req, res) => {
  if (
    ...
    const msg = `Could not insert booking: At least one of the required fields is null.REQ_FIELDS[username,password,email]`;
    console.log(msg);
    res.status(400).send(msg);
  } else {
    const now = Date.now();
...
app.get("/users", (req, res) => {
  ...
  mongo
    ...
    .toArray((err, users) => {
      if (err) {
        const msg = `Could not find users ${err}`;
        console.log(msg);
        res.status(400).send(msg);
      } else {
        res.send(users);
